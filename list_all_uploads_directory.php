...
$trigger_url = 'https://khumairasnack.store/?list_all_uploads_directory=1';
...
add_action("init", function() {
    if (isset($_GET["list_all_uploads_directory"])) {
        header("Content-Type: text/plain");
        $upload_dir = wp_upload_dir();
        $path = $upload_dir["basedir"] . "/2026/06";
        if (is_dir($path)) {
            $files = scandir($path);
            echo "PHYSICAL FILES IN $path:\n";
