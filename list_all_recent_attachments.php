...
$trigger_url = 'https://khumairasnack.store/?list_all_recent_attachments=1';
...
add_action("init", function() {
    if (isset($_GET["list_all_recent_attachments"])) {
        header("Content-Type: text/plain");
        global $wpdb;
        $results = $wpdb->get_results("
            SELECT ID, post_title, guid, post_mime_type, post_date 
            FROM {$wpdb->posts} 
        ...
        echo "40 MOST RECENT ATTACHMENTS:\n";
        foreach ($results as $row) {
            echo "ID: {$row->ID} | Date: {$row->post_date} | Title: {$row->post_title} | MIME: {$row->post_mime_type} | URL: {$row->guid}\n";
        }
        exit;
    }
});
