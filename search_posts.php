...
$trigger_url = 'https://khumairasnack.store/?search_posts=1';
...
add_action("init", function() {
    if (isset($_GET["search_posts"])) {
        ...
        echo "=== SEARCHING DATABASE FOR SNACK PRODUCTS ===\n";
        
        $terms = array("Kentang", "Mustofa", "Kriuk", "Kita", "Bawang", "Kulit", "Kuwut", "Lemon", "Sereh");
        foreach ($terms as $term) {
            $results = $wpdb->get_results($wpdb->prepare(
                "SELECT ID, post_title, post_type, post_status FROM {$wpdb->posts} WHERE post_title LIKE %s OR post_content LIKE %s",
                "%" . $term . "%",
                "%" . $term . "%"
            ));
